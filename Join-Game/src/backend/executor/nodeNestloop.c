...
#define FAILURE_COUNT_N 10 // Number of failures allowed during exploration, before jumping into next outer tuple, for exploration
#define DEBUG_FLAG 0 // print statements will be activate if set to 1

failedtuples *head;
failedtuples *tail;
failedtuples *ptr;
unsigned int failTupleAmount;

static failedtuples* 
create_node() {
    failedtuples *new_node = (failedtuples*) malloc(sizeof(failedtuples));
    if (!new_node) {
        fprintf(stderr, "Memory allocation failed\n");
        exit(EXIT_FAILURE);
    }
    new_node->tupletableslot = NULL;
    new_node->next = NULL;
    return new_node;
}
...
seedToExploitLeftPage(PlanState *pstate){
		...
		*/
		...
		if ( (outerPlan->oslBnd8RightTableCacheHead == (OSL_BND8_RIGHT_TABLE_CACHE_MAX_SIZE - 1)) || (outerPlan->oslBnd8_currExploreTupleFailureCount > FAILURE_COUNT_N) ){
			...
			if(outerPlan->oslBnd8_currExploreTupleReward>0){
				...
				else if(!TupIsNull(tail->tupletableslot)){
					tail->next = create_node();
					tail = tail->next;
				}
				tail->tupletableslot = MakeSingleTupleTableSlot(outerPlan->oslBnd8_currExploreTuple->tts_tupleDescriptor);
				ExecCopySlot(tail->tupletableslot,outerPlan->oslBnd8_currExploreTuple);
				failTupleAmount += 1;
			}
		}
	...
	while (!outerPlan->pgNst8LeftParsedFully && outerPlan->pgNst8LeftPageHead < PGNST8_LEFT_PAGE_MAX_SIZE) {
		...
		if(TupIsNull(outerTupleSlot)){
			ExecCopySlot(outerPlan->pgNst8LeftPage[outerPlan->pgNst8LeftPageHead], head->tupletableslot);
			TupleTableSlot *tmp = head;
			head = head->next;
			free(tmp);
		}
		else{
	...
	// //collect the tuples that are not
	// while(!TupIsNull(outerTupleSlot)){
	// 	if(!TupIsNull(tail->tupletableslot)){
	// 		tail->next = create_node();
	// 		tail = tail->next;
	// 	}
	// 	tail->tupletableslot = MakeSingleTupleTableSlot(outerPlan->oslBnd8_currExploreTuple->tts_tupleDescriptor);
	// 	ExecCopySlot(tail->tupletableslot,outerTupleSlot);
	// 	outerTupleSlot = ExecProcNode(outerPlan);
	// }
...
ExecEndNestLoop(NestLoopState *node)
{
	...
	*/
	// print("total tuples in LinkedList:");
	// print(failTupleAmount);
	while(head != NULL){
		TupleTableSlot *temp = head;
		head = head->next;
		free(temp);
	}
